...
class LuaClassMember(LuaObject):
    ...
    def get_index_text(self, modname: str, name_cls: str) -> str:
        ...
        elif self.objtype == 'classattribute':
            try:
                class_name, attr_name = name.rsplit('.', 1)
            except ValueError:
                if modname:
                    ...
                    return name
            if modname and add_modules:
                return _('%s (%s.%s class attribute)') % (attr_name, modname, class_name)
            else:
                return _('%s (%s class attribute)') % (attr_name, class_name)            
        else:
            return ''
...
class LuaDomain(Domain):
    ...
    object_types: Dict[str, ObjType] = {
        ...
        'classattribute': ObjType(_('attribute'), 'attr', 'obj'),
    ...
    directives = {
        ...
        'classattribute': LuaClassAttribute,
        'module': LuaModule,
