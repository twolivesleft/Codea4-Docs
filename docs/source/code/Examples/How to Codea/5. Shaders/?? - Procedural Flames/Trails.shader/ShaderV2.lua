{
    ...
    properties =
    {
        {"_warpScale", "vec2", {1.3125, 5.375} },
        {"_noiseTiling", "vec2", {3.0, 2.0} },
        {"_widthScale", "float", 1.0 },
        {"_detailNoiseTiling", "vec2", {20.0, 2.0} },
    },
    ...
    pass =
    {
        ...
        vertex =
        [[
            uniform vec2 _warpScale;
            uniform vec2 _noiseTiling;
            uniform float _widthScale;
            uniform vec2 _detailNoiseTiling;
        ...
        [[
            ...
            float sampleHeight26(vec2 uv)
            {
                return ((1.0 - smoothstep(0.05, 0.09125, (distance((uv + ((_warpScale*vec2(-psdnoise((uv * floor((_noiseTiling * max(_noiseTiling, vec2(1.0, 1.0))))), floor((_noiseTiling * max(_noiseTiling, vec2(1.0, 1.0))))).b, psdnoise((uv * floor((_noiseTiling * max(_noiseTiling, vec2(1.0, 1.0))))), floor((_noiseTiling * max(_noiseTiling, vec2(1.0, 1.0))))).g)) * 0.01)).x, 0.5)/_widthScale))) * remap(normalizeNoise(pcnoise2D(((uv * floor((max(_detailNoiseTiling, vec2(1.0, 0.0)) * 2.0))) + (floor((max(_detailNoiseTiling, vec2(1.0, 0.0)) * 2.0)) * ((_warpScale*vec2(-psdnoise((uv * floor((_noiseTiling * max(_noiseTiling, vec2(1.0, 1.0))))), floor((_noiseTiling * max(_noiseTiling, vec2(1.0, 1.0))))).b, psdnoise((uv * floor((_noiseTiling * max(_noiseTiling, vec2(1.0, 1.0))))), floor((_noiseTiling * max(_noiseTiling, vec2(1.0, 1.0))))).g)) * 0.01))), floor((max(_detailNoiseTiling, vec2(1.0, 0.0)) * 2.0)))), vec2(-0.96875, 2.22).x, vec2(-0.96875, 2.22).y, vec2(0.0, 1.0).x, vec2(0.0, 1.0).y));
            }
            
            vec3 heightToNormal26(vec2 uv, float strength, vec2 texelSize)
            {
                ...
                float s01 = sampleHeight26(uv + delta * off.xy);
                float s21 = sampleHeight26(uv + delta * off.zy);
                float s10 = sampleHeight26(uv + delta * off.yx);
                float s12 = sampleHeight26(uv + delta * off.yz);
            ...
            void material(inout MaterialInputs material)
            {
                ...
                vec3 temp_26 = heightToNormal26(getUV0(), 32.0, vec2(0.01, 0.01) * 0.1000);
                material.baseColor.rgb = remap(temp_26, vec2(-1.0, 1.0).x, vec2(-1.0, 1.0).y, vec2(0.0, 1.0).x, vec2(0.0, 1.0).y);
                vec2 floor_12 = floor((_noiseTiling * max(_noiseTiling, vec2(1.0, 1.0))));
                vec3 temp_14 = psdnoise((getUV0() * floor_12), floor_12);
                vec2 multiply_15 = ((_warpScale*vec2(-temp_14.b, temp_14.g)) * 0.01);
                vec2 floor_11 = floor((max(_detailNoiseTiling, vec2(1.0, 0.0)) * 2.0));
                float temp_18 = pcnoise2D(((getUV0() * floor_11) + (floor_11 * multiply_15)), floor_11);
                float multiply_25 = ((1.0 - smoothstep(0.05, 0.09125, (distance((getUV0() + multiply_15).x, 0.5)/_widthScale))) * remap(normalizeNoise(temp_18), vec2(-0.96875, 2.22).x, vec2(-0.96875, 2.22).y, vec2(0.0, 1.0).x, vec2(0.0, 1.0).y));
                material.baseColor.a = smoothstep(0.0, 0.1, multiply_25);
